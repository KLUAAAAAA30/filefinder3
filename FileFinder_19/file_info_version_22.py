...
def get_values_from_db(connection):
    ...
    enable_excel_file_data_scan_min_row = config_values.get(
        "ENABLE_EXCEL_FILE_DATA_SCAN_MIN_ROW"
    )
    global n_days
...
def get_values_from_env():
    ...
    global enable_excel_file_data_scan_min_row
    enable_excel_file_data_scan_min_row = os.getenv(
        "ENABLE_EXCEL_FILE_DATA_SCAN_MIN_ROW"
    )
    global n_days
    ...
    try:
        for partition in psutil.disk_partitions():
            try:
                ...
                print(
                    f"An error occurred while processing partition {partition.device}: {inner_exception}"
                )
...
def is_recently_accessed_or_modified(file_path, n_days):
    ...
    try:
        ...
        # Log the error to the log file
        logger.error(
            f"Error checking file modification/access time: {str(e)}", exc_info=True
        )
        return False
...
def create_xls_file_sheet_row_table(
    connection, xls_files, employee_username, start_time
):
    try:
        cursor = connection.cursor()
        for xls_file in xls_files:
            xls_data = pd.read_excel(
                xls_file, sheet_name=None, header=None
            )  # Read all sheets
            ...
            for sheet_name, sheet in xls_data.items():
                ...
                for row_idx in range(
                    min(int(enable_excel_file_data_scan_min_row), num_rows)
                ):  # Read up to the first 3 rows

                    is_row = (
                        "no" if row_idx == 0 else "yes"
                    )  # First row is a heading, the rest are data
                    col_data = sheet.iloc[
                        row_idx, :10
                    ].tolist()  # Take the first 10 columns
                    col_data.extend(
                        ["NULL"] * (10 - len(col_data))
                    )  # Fill the remaining columns with "NULL"
                    col_data = [
                        str(data)[:255] for data in col_data
                    ...
                    connection.commit()
        print(
            "[bright_green]Data inserted into xls_file_sheet_row table.[/bright_green]"
        )
...
def create_audit_table(
    connection, employee_username, ip, start_time, end_time, elapsed_time, scan
):
    if scan == "File Count":
        scan = "Count"
    activity = "Completed"
...
def insert_f_machine_files_summary_count(
    connection,
    ipaddress,
    hostname,
    ops,
    ...
    try:
                ...
                if sh_drive["path"]:
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            drive_names,
                            shared_folder_name,
                            truncated_shared_folder_path,
                            shared_folder_description,
                            start_time,
                            employee_username,
                            start_time,
                            employee_username,
        ...
        # print(query)
        print(
            "[bright_green]Data inserted into f_machine_files_summary_count table,d_shared_folders .[/bright_green]"
        )
        logger.success(
            "Data inserted into f_machine_files_summary_count table,d_shared_folders ."
        )

    except Exception as e:
        print("[bright_red]An error occurred during database operations.[/bright_red]")
        logger.error(
            f"Error insert_f_machine_files_summary_count: {str(e)}", exc_info=True
        )
...
def windows(connection):
    ...
    if not drives:
        ...
        scan_option = select(
            "Select the type of scan:", choices=scan_option_choices
        ).ask()

        try:
            if scan_option == "All Drive Scan":
                print(
                    f"Performing a full scan for data files modified or accessed in the last {n_days} days."
                )
                print("***")
                ...
                logger.success(
                    "The File Scanning is now complete!! Please wait while we insert the data into the database."
                )
            elif scan_option == "Specific Drive Scan":
                drive_choice = input(
                    r"Enter the drive letter to scan (e.g., C:\, D:\, E:\, ...) or drive path (e.g., C:\Users\Username): "
                ).upper()

                # if drive_choice in [d[0] for d in drives]:
                # selected_drive = [d for d in drives if d[0] == drive_choice][0]
                print(
                    f"Scanning {drive_choice} for data files modified or accessed in the last {n_days} days:"
                )
                print("***")
                print(
                    "Windows Scan: The Tool is now counting For  Files. Please Wait..."
                )
                found_assets = search_files(
                    drive_choice,
                    d_file_details_file_extensions,
                    n_days,
                    sensitive_patterns,
                ...
                print(
                    "[bright_green]Windows Scan: The Tool has completed count For  Files. Please Wait...[/bright_green]"
                )
                logger.success(
                    "Windows Scan: The Tool has completed count For  Files. Please Wait..."
                )
                # else:
        ...
        try:
            ...
            print(
                "[bright_green]Windows Scan: insert_f_machine_files_summary_count in progress. Please Wait...[/bright_green]"
            )
            logger.success(
                "Windows Scan: insert_f_machine_files_summary_count in progress. Please Wait..."
            )
            ...
            print(
                "[bright_green]Windows Scan: insert_f_machine_files_summary_count complete. Please Wait...[/bright_green]"
            )
            logger.success(
                "Windows Scan: insert_f_machine_files_summary_count complete. Please Wait......"
            )
            ...
            if found_assets:
                ...
                logger.info(
                    "Scan results for the last {n_days} days saved to the MySQL database."
                )
                print(
        ...
        if enable_excel_file_data_scan == "true":
            if ".xls" or ".xlsx" in d_file_details_file_extensions:
                ...
                xls_files = [
                    file for file in found_assets if file.lower().endswith(extension)
                ]
                if xls_files:
                    create_xls_file_sheet_table(
                        connection, xls_files, employee_username, start_time
                    )
                    create_xls_file_sheet_row_table(
                        connection, xls_files, employee_username, start_time
                    )
                    connection.close()
                else:
...
def linux(connection):
    ...
    try:
        ...
        if scan_option == "All Drive Scan":
            ...
            found_assets.extend(
                search_files(
                    root_dir, d_file_details_file_extensions, n_days, sensitive_patterns
                )
            )
            print(
                "[bright_green]The File Scanning is now complete!! Please wait while we insert the data into the database...[/bright_green]"
            ...
            print(
                f"Scanning {path_choice} for data files modified or accessed in the last {n_days} days:"
            )
            print("***")
            print("The Tool is now scanning for Data Files. Please Wait...")
            found_assets = search_files(
                path_choice, d_file_details_file_extensions, n_days, sensitive_patterns
            )
            print(
                "[bright_green]The File Scanning is now complete!! Please wait while we insert the data into the database...[/bright_green]"
            )
        ...
        try:
            ...
            if found_assets:
                for asset in found_assets:
                    upsert_to_database(asset, connection, employee_username, start_time)
                print(
                    f"Scan results for the last {n_days} days saved to the MySQL database."
                )
            else:
        ...
        print(
            "[bright_red]An error occurred during the scan and database operations.[/bright_red]"
        )
...
def insert_log_file_to_mysql(
    connection, log_folder, ip_address, hostname, employee_username, start_time
):
    try:
        ...
        if os.path.exists(log_file_path):
            ...
            connection.commit()
            print(
                "[bright_green]Log file content inserted into app_log_file table.[/bright_green]"
            )
        else:
            print(
                "[bright_yellow]Log file not found. Skipping insertion.[/bright_yellow]"
            )
...
if __name__ == "__main__":
    ...
    if os.getenv("ENABLE_APP_LOG_TO_DB") == "true":
        insert_log_file_to_mysql(
            connection, log_folder, ipaddrs, hostname, employee_username, start_time
        )
